...
class VehiclesCRUD {
  ...
  generateRequestProcessorMap() {
    return {
      'Vehicles': {
        "emigateway.graphql.query.GeneratorVehiclesListing": { fn: instance.getGeneratorVehiclesListing$, instance, jwtValidation: { roles: READ_ROLES, attributes: REQUIRED_ATTRIBUTES } },
        "emigateway.graphql.query.GeneratorVehicles": { fn: instance.getVehicles$, instance, jwtValidation: { roles: READ_ROLES, attributes: REQUIRED_ATTRIBUTES } },
        "emigateway.graphql.mutation.GeneratorCreateVehicles": { fn: instance.createVehicles$, instance, jwtValidation: { roles: WRITE_ROLES, attributes: REQUIRED_ATTRIBUTES } },
        "emigateway.graphql.mutation.GeneratorUpdateVehicles": { fn: instance.updateVehicles$, jwtValidation: { roles: WRITE_ROLES, attributes: REQUIRED_ATTRIBUTES } },
        "emigateway.graphql.mutation.GeneratorDeleteVehicless": { fn: instance.deleteVehicless$, jwtValidation: { roles: WRITE_ROLES, attributes: REQUIRED_ATTRIBUTES } },
      }
    }
  };
  ...
  getGeneratorVehiclesListing$({ args }, authToken) {
    ...
    return forkJoin(
      VehiclesDA.getVehiclesList$(filterInput, paginationInput, sortInput).pipe(toArray()),
      queryTotalResultCount ? VehiclesDA.getVehiclesSize$(filterInput) : of(undefined),
    ).pipe(
  ...
  */
  createVehicles$({ root, args, jwt }, authToken) {
    ...
    return VehiclesDA.createVehicles$(aggregateId, input, authToken.preferred_username).pipe(
      mergeMap(aggregate => forkJoin(
        CqrsResponseHelper.buildSuccessResponse$(aggregate),
        eventSourcing.emitEvent$(instance.buildAggregateMofifiedEvent('CREATE', 'Vehicles', aggregateId, authToken, aggregate), { autoAcknowledgeKey: process.env.MICROBACKEND_KEY }),
        broker.send$(MATERIALIZED_VIEW_TOPIC, `GeneratorVehiclesModified`, aggregate)
      )),
  ...
  updateVehicles$({ root, args, jwt }, authToken) {
    ...
    return (merge ? VehiclesDA.updateVehicles$ : VehiclesDA.replaceVehicles$)(id, input, authToken.preferred_username).pipe(
      mergeMap(aggregate => forkJoin(
        CqrsResponseHelper.buildSuccessResponse$(aggregate),
        eventSourcing.emitEvent$(instance.buildAggregateMofifiedEvent(merge ? 'UPDATE_MERGE' : 'UPDATE_REPLACE', 'Vehicles', id, authToken, aggregate), { autoAcknowledgeKey: process.env.MICROBACKEND_KEY }),
        broker.send$(MATERIALIZED_VIEW_TOPIC, `GeneratorVehiclesModified`, aggregate)
      )),
      map(([sucessResponse]) => sucessResponse),
      catchError(err => iif(() => err.name === 'MongoTimeoutError', throwError(err), CqrsResponseHelper.handleError$(err)))
  ...
  deleteVehicless$({ root, args, jwt }, authToken) {
    ...
    return forkJoin(
      ...
      from(ids).pipe(
        mergeMap(id => eventSourcing.emitEvent$(instance.buildAggregateMofifiedEvent('DELETE', 'Vehicles', id, authToken, {}), { autoAcknowledgeKey: process.env.MICROBACKEND_KEY })),
        toArray()
      )
    ).pipe(
      map(([ok, esResps]) => ({ code: ok ? 200 : 400, message: `Vehicles with id:s ${JSON.stringify(ids)} ${ok ? "has been deleted" : "not found for deletion"}` })),
      mergeMap((r) => forkJoin(
        CqrsResponseHelper.buildSuccessResponse$(r),
        broker.send$(MATERIALIZED_VIEW_TOPIC, `GeneratorVehiclesModified`, { id: 'deleted', name: '', active: false, description: '' })
      )),
