...
class VehiclesES {
    ...
    generateEventProcessorMap() {
        return {
            'Vehicles': {
                "VehiclesModified": { fn: instance.handleVehiclesModified$, instance, processOnlyOnSync: true },
            }
        }
    };
    ...
    handleVehiclesModified$({ etv, aid, av, data, user, timestamp }) {
        ...
        return iif(
            ...
            tap(() => ConsoleLogger.i(`VehiclesES.handleVehiclesModified: ${data.modType}: aid=${aid}, timestamp=${timestamp}`))
        )
    }
}
