...
function TodoSidebarContent(props) {
    ...
    const { filters: { active: activeChecked } } = useSelector(({ VehiclesManagement }) => VehiclesManagement.vehicless);
    const T = new MDText(i18n.get(user.locale));


    function handleActiveChange(evt) {
        if (activeChecked === null) {
            dispatch(Actions.setVehiclessFilterActive(true));
        } else if (activeChecked) {
            dispatch(Actions.setVehiclessFilterActive(false));
        } else {
            dispatch(Actions.setVehiclessFilterActive(null));
        }
    }
                ...
                <div className="p-24">
                    <FormGroup row>
                        <FormControlLabel
                            ...
                            label={T.translate("vehicless.filters.active")}
                        />
                    </FormGroup>
                </div>
